...
Pour vérifier si la division aléatoire des données influençait les performances, nous avons répété notre expérience avec plusieurs graines ($123$ et $545$). Les résultats présentés ci-dessus proviennent de la graine $42$. Les performances obtenues pour les méthodes calibrées sont présentées ci-dessous :
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        \hline
        $s_1$ + non expert & $87.25$ & $> 3.8415$ & Rejet de $H_0$ \\
        $s_1$ + expert & $1.05$ & $< 3.8415$  & Conserve $H_0$ \\
        $s_2$ + non expert & $282.54$ & $> 3.8415$ & Rejet de $H_0$ (trop élevée) \\
...
Si nous intégrions notre travail à partir du quantile que nous avons calculé à l'aide du deuxième score (score cumulatif) sur les données expertes, nous aurions obtenu, si nous reprennons l'exemple de la \autoref{fig:prediction_sets}, alors nous obtiendrons une taille de $1$ seule espèce prédite pour chacune des observations. Cela s'explique par le fait qu'il n'y a que la première prédiction dont la probabilité était supérieure à $1 - 0,5614$. Cependant, pour la dernière plante, étant donné que la photographie est de mauvaise qualité, ce n'est pas la bonne espèce qui est prédite (elle fait partie de $5\%$ d'erreur).
