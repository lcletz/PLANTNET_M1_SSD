...
Puisque l'échantillon étudié est moins conséquent que celui de la première étude, $\num{3000}$ observations ont été aléatoirement sélectionnées, soit $\num{1000}$ observations supplémentaires, pour une meilleure intuition. 
...
Outre ces deux espèces, nous remarquons qu'il y a plusieurs espèces aux scores très proches de $0$ qui ont été choisies, soit parce que l'IA n'était pas confiante et a proposée de nombreuses options ayant des probabilités minimes, soit parce que les votants ont eux-mêmes été chercher une espèce en particulier parmi les espèces les moins évidentes selon l'IA.
...
\begin{figure}[H]
    ...
    \caption{Représentation de nos quatre modèles}
    \label{models}
  \end{figure}
...
\begin{figure}[H]
    \centering
    \makebox[\linewidth][c]{
        \includesvg[width=1.2\linewidth]{images/quantiles.svg}
    }
    \captionsetup{skip=-20pt}
    \caption{Conservation des observations en fonction des quantiles des méthodes}
    \label{fig:quantiles}
\end{figure}
...
Le tableau met en évidence les quantiles obtenus selon différentes configurations :
\begin{itemize}
    \item \textbf{Score s1 + données non expertes :} quantile le plus élevé ($0,9990$), traduisant une approche plus permissive qui conserve presque toutes les observations (mis à part celles pour lesquelles nous avons attribuée un score de $0,0001$ car elles n'avaient pas la bonne étiquette de proposée).
    \item \textbf{Score s1 + données expertes :} quantile de $0,9649$, témoignant d’une amélioration en termes de sélectivité avec un peu plus d'observations non prises en compte.
...
Les \autoref{fig:quantiles} illustre la répartition des observations testées selon les scores s1 et s2, en distinguant les données expertes et non expertes.
...
L’analyse montre que le score s2 tend à réduire la proportion d’observations acceptées, en particulier lorsqu’il est appliqué aux données expertes. Cela suggère une méthode plus rigoureuse, favorisant un filtrage plus strict.
