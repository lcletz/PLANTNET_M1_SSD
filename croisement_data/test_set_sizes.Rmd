...
The quantile for the method "s1 + non-experts" is 0,999 then:
```{r}
size_1 <- sapply(proba_list, function(p) sum(p > 1-0.999))
```
...
The quantile for the method "s2 + non-experts" is 0,7538, then:
```{r}
size_3 <- sapply(proba_list, function(p) sum(p > 1-0.7538))
```

The quantile for the method "s2 + experts" is 0,5614, then:
```{r}
size_4 <- sapply(proba_list, function(p) sum(p > 1-0.5614))
```
...
interest_values <- size_3_df %>%
  count(count) %>%
  filter(count > 2 | count == 0)
