...
data <- fromJSON(file.path(chemin, "5_scores_nonexp.json"), 
...
p1 <- ggplot(df, aes(x = one_minus_prob)) +
  ...
  scale_x_continuous(
   ...
   scale_y_continuous(
    ...
    minor_breaks = seq(0, 8000, by = 500)
  ) +
  theme_bw() +
  labs(
...
p2 <- ggplot(df, aes(x = sum_until_correct)) +
  geom_histogram(bins = 30, fill = "pink", color = "black", alpha = 0.7) +
   scale_x_continuous(
     breaks = seq(0, 3, by = 0.5),        # Graduations principales tous les 0.1
     minor_breaks = seq(0, 3, by = 0.25)  # Graduations mineures tous les 0.05
     #limits = c(0,1)
   ) +
   scale_y_continuous(
    breaks = seq(0, 70000, by = 5000),
    minor_breaks = seq(0, 70000, by = 2500)
  ) +
  theme_bw() +
  labs(
